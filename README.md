...
| `/lacquer` | 灰胎髹漆遍次 | 按遍次累加厚度、荫房温湿度窗口校验、层积条与养护天数；某遍打磨不到位可登记返工（返工日期/返工人/一句原因，遍次号不变、次数累计），返工后本遍按新厚度计并从该遍起重算累计厚度；已上弦的琴须先撤掉上弦记录 |
...
- `db.version(1)` 建表声明索引；`db.version(2).upgrade(...)` 为髹漆表增加 `[guqinNo+seq]` 复合索引并回填历史厚度；`db.version(3).upgrade(...)` 为髹漆遍次补齐内嵌的返工记录数组 `reworks`（返工日期/返工人/原因/新厚度）。升级前可用顶栏「导出备份」导出全量 JSON。
